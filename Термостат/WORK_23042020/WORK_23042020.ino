//Нужно решить проблемму остающейся единице на дисплее при опускание ниже 100
...
byte Rft=22;//resistor of devider
byte DR=3; //gate
float f1024=1024;
byte kb_delay=200; //holding kb time
byte Ru_off=100; // overheat
int dtimer=300;//frequency of switch
...
long unsigned tim1;
byte i=0;
long unsigned dtim=300000;      // не повысилось на градус за 5 минут
...
if(n=='0')
        {
          ...
          myOLED.print("Set",OLED_L,1);
          myOLED.print("        ",OLED_R,1);
          myOLED.print("Current",OLED_R,1);
          myOLED.print("     ",OLED_L,6);
          myOLED.print("R_set",OLED_L,6);
          myOLED.print("     ",OLED_C,6);
          myOLED.print("T_tc",OLED_C,6);
          myOLED.print("     ",OLED_R,6);
          myOLED.print("R_cur",OLED_R,6);
          int i;
          for(i=0;i<boxnum;i++) 
...
if(n=='E')
      {
        ...
        myOLED.setFont(MediumFontRus);
        myOLED.clrScr(); 
        myOLED.print( "Reading error"     , OLED_C, 2);
        myOLED.print( "calibration"     , OLED_C, 3);
        }
         if(dat==2)
        {
           myOLED.setFont(SmallFontRus);
            myOLED.print( "     " , OLED_R, 1); 
            myOLED.print( "     " , OLED_L, 1); 
           myOLED.print( "!Out of table!", OLED_C, 1); }
        ...
        if(dat==3)
        {
           myOLED.setFont(SmallFontRus);
            myOLED.print( "    " , OLED_R, 1); 
            myOLED.print( "    " , OLED_L, 1); 
           myOLED.print( "!R OUT OF TABLE!", OLED_C, 1); 
...
if(n=='K')
      {
        if(dat==0)
          {  myOLED.clrScr(); 
          // myOLED.setFont(SmallFontRus);
           myOLED.print( "Calibration" , OLED_C, 1); 
           myOLED.print( "Is over" , OLED_C, 2); 
           ...
           myOLED.print("t1",OLED_R,3);
            ...
            if(dat==1)
            {
              myOLED.setFont(SmallFontRus);
            myOLED.clrScr();                                   
            myOLED.print( "CALIBRATION"    ,   OLED_C, 0);   
            myOLED.print( "TEMPERATURE"     , OLED_C, 1);    
            myOLED.print( "TIME" , OLED_L, 3); 
            myOLED.print( "NUMBER" , OLED_R, 3);   
            myOLED.print( "RESISTANCE(Ohm/div)"     , OLED_C, 5);    
            }
            if(dat==3)
          {            
             myOLED.setFont(SmallFontRus);
          myOLED.clrScr(); 
        myOLED.print( "CALIBR" , OLED_C, 3); 
        myOLED.print( "TABLE" , OLED_C, 4); 
        myOLED.print( "UNAVELIBLE" , OLED_C, 5); 
        delay(2000);
        myOLED.clrScr(); 
        myOLED.print( "HOLD KB" , OLED_C, 3); 
        myOLED.print( "FOR CALIBRATION" , OLED_C, 5); 
        delay(2000);
        myOLED.clrScr(); 
        myOLED.print( "HOLD" , OLED_C, 3); 
        myOLED.print( "VALCODER" , OLED_C, 4); 
        myOLED.print( "TO WORK IN DIV" , OLED_C, 5); 
        delay(2000);
          }
                }
if(n=='i')
