...
unsigned long tm1,tm2,dtd;
float T,dT;
long dTm=3000;//blinking time
byte f=9;//blinking nomber
int t1m;
byte fn;
long dTee=10000; //delay before puting to eepromt
...
{ tm2=millis();
  fch[0]=display.encodeDigit(floor(d/100));
  fch[1]=display.encodeDigit(floor(d/10)-fch[0]*10);
  fch[2]=display.encodeDigit(floor(d)-floor(d/10));
  fch[3]=display.encodeDigit((d-floor(d))*10);
  if(tm2-tm1>t1m*fn)
 {if(b_flag==0)
 {fch[n]=0;
 b_flag=1;}
